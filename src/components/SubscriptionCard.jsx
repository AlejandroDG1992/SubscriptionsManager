...
const SubscriptionCard = ({
  ...
  return (
    <div
    className="text-gray-900 bg-[#fff] border-2 border-[#EAEBED] dark:text-white dark:border-gray-700 p-4 max-w-md mx-auto dark:bg-gradient-to-r from-slate-900 to-slate-800 relative rounded-3xl dark:bg-gray-800 text-gray-200 p-5"
    ...
    >
      ...
      <div className="text-[8px] absolute right-5 bottom-0">{subscription.tags?.name}</div>

      {hoveredCardId === subscription.id && (
        <button onClick={() => setOpenMenuId(subscription.id)} className="absolute top-[5px] right-[10px]">⋮</button>
      )}
